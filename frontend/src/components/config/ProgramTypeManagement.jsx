...
const ProgramTypeManagement = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch(`${API_URL}/tickets/config/program-types`, {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to add program type');
      }
  ...
  const handleUpdate = async (id) => {
    ...
    setSuccess(null);
    try {
      const response = await fetch(`${API_URL}/tickets/config/program-types/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to update program type');
      }
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`คุณต้องการลบประเภทโปรแกรม "${name}" ใช่หรือไม่?`)) return;
    setError(null);
    setSuccess(null);
    try {
      const response = await fetch(`${API_URL}/tickets/config/program-types/${id}`, {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to delete program type');
      }
      await fetchItems();
      setSuccess('ลบประเภทโปรแกรมสำเร็จ');
      setTimeout(() => setSuccess(null), 3000);
    } catch (err) {
      setError(err.message);
    }
...
export default ProgramTypeManagement;
